...
import { LmsPageQuery } from '../graphql/query/lmsPageQuery';

export const getStaticProps = async () => {
  const data = await request(process.env.NEXT_PUBLIC_GRAPH_API, LmsPageQuery);

  return { props: data, revalidate: 60 };
};

function lms(props) {
  const { homePage, lm } = props;
  return (
    <div>
      <Head>
        ...
        <title>Vryno - Loan Management System</title>
      ...
      />
      <main>
        <LmsHero content={lm.Hero} />
        <LmsFeature content={lm.LMSFeature} />
        <LmsWorking content={lm.LMSWorking} />
      </main>
